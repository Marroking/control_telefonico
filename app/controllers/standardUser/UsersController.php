...
class UsersController extends \BaseController {
	...
	*/
	...
	public function update($id)
	{
		...
		{
			$input = Input::only('first_name', 'last_name', 'employee_id', 'radio', 'email');

			$this->usersEditForm->excludeUserId($user->id)->validate($input);

			$user->fill($input)->save();

			return Redirect::route('profiles.edit', $user->id)->withFlashMessage('¡El Usuario se ha Actualizado exitosamente!');
		}
		...
		else
		{
			...
			return Redirect::route('profiles.edit', $user->id)->withFlashMessage('¡El Usuario (y la Contraseña) se han Actualizado exitosamente!');
		}
	}
